...
public class OrderController implements IOrderController, Observer {
    ...
    @Override
    public void OnOrder(String Cus, String add, int dc, List<OrderDetails> list, CCinfo cardInfo) {
        ...
        if (orderCode == 6) {
            view.OrderError("מספר תעודת זהות לא תקין");
        }
        if (orderCode == -1) {
            model.addOrder(od);
        }
        ...
        private int isValid (CCinfo card, String address){
            ...
            if(card.getIdNum().length()<8) return 6;
            return -1;
        }
